        ...
        <div class="card-body">
          ...
          <table id="table1" class="table table-bordered table-striped" style="font-size: 12px">
            ...
            <?php foreach($fa as $s => $row) {?>  
              <tr align="center">
                <td><?= date('d-m-Y', strtotime($row->tanggal_pelaksanaan_jahit)); ?></td>                
                <td><?= $row->nomor_so; ?></td>
                ...
                <?php if( $row->id_jadwal_jahit == $id_jadwal_max[$nilai] and $row->status_jahit != "jahit") {?>  
                          <a href="<?=site_url()?>finishing/FinishingAkhir/tambah_jadwal_fa_jahit/<?= $row->id_jahit; ?>">
                            <i class="fa fa-plus" style="font-size:18px;margin-right: 20px;"></i>
                          </a>  
                          <?php } ?>    

                  <a href="<?=site_url()?>finishing/FinishingAkhir/edit_jadwal_fa_jahit/<?= $row->id_jahit; ?>">
                    <i class="fa fa-pencil" style="font-size:18px;margin-right: 20px;"></i>
                  </a>
                  <a href="<?=site_url()?>finishing/FinishingAkhir/lihat_jadwal_fa_jahit/<?= $row->id_jahit; ?>">
                    <i class="fa fa-eye" style="font-size:18px;"></i>
                  </a>

                   <!-- hapus -->
                   <a style="margin-left:20px;" href="<?=site_url('finishing/FinishingAkhir/hapus_jahit/'.$row->id_jahit."-".$row->id_order)?>" onclick="return confirm('Apakah Anda Yakin')" class="btn btn-danger btn-xs">
